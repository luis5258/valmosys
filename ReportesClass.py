...
class RepDiarioPromedio(QDialog):
    ...
    def HacerRep(self):
        ...
        for ren, item in enumerate(Data):
            ...
            NoAnimalesActual=CantidadActualAnimales(self,Data[ren][6],FechaFinal)
        ...
        ListaTem.append("{:,}".format(Prod1))
        ListaTem.append("{:,}".format(Prod2))
        ListaTem.append("{:,}".format(Prod3))
        ListaTem.append("{:,}".format(Prod4))
        ListaTem.append("{:,}".format(Prod5))
        ListaTem.append("{:,}".format(TotProd))
    ...
    def PrintToPdf(self):
        ...
        TitulosCols=['Corral','Alfa100%','Fase 1', 'Fase 2','Fase 3','FaseInt','TotServido','NoAnimAct','DiasAnim','PromDia']
        Datos.insert(0,TitulosCols)
        EncabeData=['Fecha Reporte:',FechaPre,'Cliente:',clienteDescrip,'Fecha Inicial:',FechaInicial,'Fecha Final:',FechaFinal]
        if len(Datos) < 40 :
            #r=GeneraReporte(self,3,TituloRep,EncabeData,list(Datos),[''],['']) 
            r=GeneraRepDiarioLargo(self,3,TituloRep,EncabeData,list(Datos))   
...
class RepAnimaClienXCorral(QDialog):
    ...
    def Hacer(self):
        ...
        TemList=[]
        RepData=[]
        if self.ui.RBotPorCliente.isChecked():
            RepData=run_query2(self,query,(FechaIniMenos1,FechaIniMenos1,FechaInicial,FechaFinal,FechaInicial,FechaFinal,idcliente,idcliente))
                   
        elif self.ui.RBotTodos:
            clientes=ClientesConCorralAsignado(self)
            for ren in clientes:
                d=run_query2(self,query,(FechaIniMenos1,FechaIniMenos1,FechaInicial,FechaFinal,FechaInicial,FechaFinal,ren[0],ren[0]))
                for j in d:
                    RepData.append(j)
...
class RepAnimalesClienteCorraCantAct(QDialog):
    ...
    def HacerReporteUnCliente(self,IDCliente,Fecha):
        query=  """ SELECT  tblClientes.Nombre AS CLIENTE, tblCorrales.Descripcion AS CORRAL,
                    ...
                    INNER JOIN tblClientes ON tblClientes.ID = tblMovimientoAnimales.IDCliente 
                    INNER JOIN tblCorrales ON  tblCorrales.ID = tblMovimientoAnimales.IDCorral
                    WHERE  tblMovimientoAnimales.IDCliente = ? AND tblMovimientoAnimales.IDCorral IN (SELECT ID FROM tblCorrales WHERE AsignACliente = ? ) 
                    GROUP BY tblMovimientoAnimales.IDCorral,CLIENTE 
                """
        Datos=run_query2(self,query,(Fecha,Fecha,IDCliente,IDCliente)) 
        Total=0 ; Data=[]
        for ren in range (0,len(Datos)):
