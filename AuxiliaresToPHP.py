...
def ReporteDiario(cliente,FechaInicial,FechaFinal):
    ListaForRep=[] ; ListaTem=[]
    IDListaProd=ProductosQueSeSirven(1)
    query= """ SELECT tblCorrales.Descripcion,
                      SUM ( CASE WHEN tblOrdenesSurtido.IDProducto = ?    THEN tblDetalleOrdenSurtido.CantSurtida ELSE 0 END),
                      SUM ( CASE WHEN tblOrdenesSurtido.IDProducto = ?    THEN tblDetalleOrdenSurtido.CantSurtida ELSE 0 END),
                      SUM ( CASE WHEN tblOrdenesSurtido.IDProducto = ?    THEN tblDetalleOrdenSurtido.CantSurtida ELSE 0 END),
                      SUM ( CASE WHEN tblOrdenesSurtido.IDProducto = ?    THEN tblDetalleOrdenSurtido.CantSurtida ELSE 0 END),
                      tblDetalleOrdenSurtido.IDCorral
               FROM tblDetalleOrdenSurtido
    ...
    for ren, item in enumerate(Data):
        ...
        DiasAnimal=CalculaDiasAnimal(cliente,item[-1],FechaInicial,FechaFinal)
